...
class ReportEpcService
{
    ...
    public function reportPRGuarantee(Anlage $anlage):array
    {
        if (self::getCetTime('object') < $anlage->getFacDateStart()) {
            $startYear = $anlage->getPacDate()->format('Y');
            $endYear = $anlage->getPacDateEnd()->format('Y');
            $anzahlMonate = ((int)$anlage->getPacDateEnd()->format('Y') - (int)$anlage->getPacDate()->format('Y')) * 12 + ((int)$anlage->getPacDateEnd()->format('m') - (int)$anlage->getPacDate()->format('m'));
        } else {
            $startYear = $anlage->getFacDateStart()->format('Y');
            $startYear = $anlage->getPacDate()->format('Y');
            $endYear = $anlage->getFacDate()->format('Y');
            $anzahlMonate = ((int)$anlage->getFacDate()->format('Y') - (int)$anlage->getFacDateStart()->format('Y')) * 12 +((int)$anlage->getFacDate()->format('m') - (int)$anlage->getFacDateStart()->format('m'));
            $anzahlMonate = ((int)$anlage->getFacDate()->format('Y') - (int)$anlage->getPacDate()->format('Y')) * 12 +((int)$anlage->getFacDate()->format('m') - (int)$anlage->getPacDate()->format('m'));
        }
        ...
        // PLD für Realene Zeitraum (PAC bis zum Auswertungs Stichtag
        // PR Abweichung für das Jahr berechen -> Daten für PR Forecast
    ...
    public function reportYieldGuarantee(Anlage $anlage):array
    {
        ...
        for ($run = 1; $run <= 2; $run++) {
            ...
            for ($n = 1; $n <= $anzahlMonate+1; $n++) {
                ...
                $guaranteedExpexted = $expectedYield * (1 - ($anlage->getTransformerTee() / 100)) * (1 - ($anlage->getGuaranteeTee() / 100));
                ...
                if ($run === 2) {// Monatswerte berechnen
                    ...
                    $report[0][] = [
                        ...
                        'minusExpected'         => $this->format(($eGridReal / $expectedYield * 100) - 100),
                        'currentMonthClass'     => $currentMonthClass,
                    ];
                }
                $month++;
        ...
        $pld = (($guaranteedExpectedEnergy - ($measuredEnergy / ($availability / 100))) / $expectedEnery) * 100 * $anlage->getPldYield();
        ...
        $report[1] = [
            ...
            [
                ...
                'unit'          => 'kWh/year',
                'explanation'   => 'Measured Energy - guaranteed Expected Energy',
            ],
            [
                'parameter'     => 'Percent Difference Calculation',
                'value'         => $this->format($percentDiffCalulation),
                'unit'          => '%',
                'explanation'   => '(Measured Energy - Expected Energy) x 100 / Expected Energy',
            ],
            [
                'parameter'     => 'Ratio',
                'value'         => $this->format($ratio),
                'unit'          => '%',
                'explanation'   => 'Measured Energy x 100 / Expected Energy',
            ],
        ];
        ...
        //je nachdem welche Formel für die PLD Berecnung genutrzt werden soll
        switch ($anlage->getPldDivisor()){
            case 'guaranteedExpected':
                ...
                break;
            default:
                $pld = (($guaranteedExpectedEnergy - ($measuredEnergy / (round($availability,2) / 100))) / $expectedEnery) * 100 * $anlage->getPldYield();
        }
        $diffdCalculation = $measuredEnergy - $guaranteedExpectedEnergy;
        $percentDiffCalulation = ($measuredEnergy - $expectedEnery) * 100 / $expectedEnery;
        $ratio = $measuredEnergy * 100 / $expectedEnery;
