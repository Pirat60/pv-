<?php
/**
 * Lade nochmal alle Wetter Daten von UP, des angegbenen Tages nach
 * (zum schließen von Lücken in den Wetterdaten)
 *
 * Wird aus der Menüsection 'Tools' aufgerufen
 */
...
#
            if ($wsstatus == "old") {
                ...
                foreach ($spalte as $out) {
                    ...
                    g4nLog( "OLD -> $sqlstamp -- $at_avg | $pt_avg | $gi_avg | $gmod_avg | $wind");
                ...
                foreach ($spalte as $out) {
                    ...
                    g4nLog( "NEW ->  $sqlstamp -- $at_avg | $pt_avg | $gi_avg | $gmod_avg | $wind");
...
#
            ...
            foreach ($sql_array as $row) {
                ...
                $stmt->bind_param('ssssssss', $row['anl_id'], $row['anl_intnr'], $row['stamp'], $f1a, $f1, $f2, $f3, $f4);
                $stmt->execute();
            }
