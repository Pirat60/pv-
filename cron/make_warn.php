...
if ( date( 'G', g4nTimeCET() ) >= 7 AND date( 'G', g4nTimeCET() ) <= 23 ) {
    ...
    if ( $resAnlagen->num_rows > 0 && true) {
        while ( $rowAnlage = $resAnlagen->fetch_assoc() ) { // Für jede Anlage
            ...
            if ($AuftragNr != 9999) {
                ////////////// Alert Code DC Soll warn_code = 4, 5, 6, 7 [START]
                if ($AnlageHatDCGruppe == "Yes" && true) {
                    ...
                    while ($ro = $resi->fetch_assoc()) {
                        ...
                        $warnView = 5; // Typ der Fehlermeldung (hier DC Power ???) soll pro Tag nur einmal erscheinen
                    ...
                    $warnView = 6; // Typ der Fehlermeldung (hier Spannung & Strom ???) soll pro Tag nur einmal erscheinen
                    $view = "dcall";
                    $range = "15";
                ...
                if ($groupsAC  && true) {
                    ...
                    foreach ($groupsAC as $groupValue) {
                        ...
                        $ArrACTGroup[$d] = actviewdatachartgroup_sqltest($andbase, $dbtableist, $from, $to, $anlgrpcount, $invnr, $min, $max); //? Wo kommt das $d her ????
                        ...
                        if ($AnlageHatACGruppe == "Yes") {
                            ...
                            g4nLog("AC Quota Berechnung: AnlageHatACGruppe == No bei $invnr");
                            $sqlExpData = "SELECT DISTINCT `stamp` AS istdate, sum(`wr_pac`) as 'exppac','grp_id' FROM $andbase.$dbtablesoll WHERE `stamp` BETWEEN \"$from\" and \"$to\" LIMIT 1 "; #GROUP by `stamp`
                            g4nLog("AC Quota Berechnung: $sqlExpData");
                        }
                        ...
                        //echo "<i>AC ->GPID: $grpid -> ACT_MW: $actPowerAc -> EXP_MW: $expPowerAc -> QUO_MW: $quotamw -> $warninfo || QUO -> $ca</i><bR>";

                    }

                    $warnView = 4; // Typ der Fehlermeldung (hier Quota at Group ??) soll pro Tag nur einmal erscheinen
                    if ($ca >= 1) {
                        $alertprioquo = "6";
                ...
                if (true) {
                    ...
                    if ($anlinputdaily == "No") {
                        if ($stamp_div_min_ws < 60) {
                            ...
                            $alertcodews = "AC002B"; // orange = Warnung (Abweichung zwischn 1 und 3 Stunden
                            $alertpriows = "2";
                ...
                if (true) {
                    ...
                    g4nLog("Anlagen ID: $anlid - PR: $prdiff");
                    $dup8 = checkdbentry($anlid, $NTimePastday, $warnView);
                    if (!$dup8) {
                        $sql_ins_al3 = "INSERT INTO db_anl_warn set stamp = '$NTimePastday', anl_id = '$anlid', warn_code = '$alertcodepr', warn_create_date = CURRENT_TIMESTAMP, warn_prio ='$alertpriopr', warn_active = 'yes', warn_view = '1', warn_count = '1', warn_info = '$prdiff'";
                        $conn->query($sql_ins_al3);
                    } else {
                        $sql_upd_al3 = "UPDATE db_anl_warn set stamp = '$NTimePastday', anl_id = '$anlid', warn_code = '$alertcodepr', warn_create_date = CURRENT_TIMESTAMP, warn_prio = '$alertpriopr', warn_active = 'yes', warn_view = '1', warn_count ='2', warn_info = '$prdiff' WHERE warn_id = '$dup8'";
